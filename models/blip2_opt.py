...
class Blip2OPT(Blip2Base):
    ...
    def __init__(self,cfg):
        ...
        #print("self.opt_tokenizer:",self.opt_tokenizer.special_tokens_map)
    ...
    def forward(self, batch):
        graphs1, graphs2, smiles_prompt_tokens, output_tokens = batch
        ...
        attention_mask = torch.cat([smiles_prompt_tokens.attention_mask, output_tokens.attention_mask], dim=1)

        # for output, ddie 
        empty_targets = torch.ones(smiles_prompt_tokens.attention_mask.shape, dtype=torch.long).to(device).fill_(-100)
    ...
    def generate(
        ...
        """
        # load drug1
        #graphs1, graphs2, mol_prompt_tokens, output_tokens, text = batch
        graphs1, graphs2, mol_prompt_tokens = batch
    ...
    def probe_qformer(
        ...
        with self.maybe_autocast():
            ...
            knn_decode_strings = np.asarray(knn_decode_strings).reshape(B, num_q,
                                                                        topk).tolist()  # shape = [mol_num, num_query_token, topk]
            knn_decode_strings = [[' '.join(ii) for ii in i] for i in
                                  knn_decode_strings]  # shape = [mol_num, num_query_token]
            if False:
