    ...
    def forward(self, *argv):
        ...
        if self.cat_grep:
            ...
            batch_mask = torch.cat(
                [torch.ones((batch_mask.shape[0], 1), dtype=torch.long, device=batch.device), batch_mask], dim=1)
            return batch_node, batch_mask
        else:
            return batch_node, batch_mask, h_graph


class GINConv_STM(MessagePassing):
    def __init__(self, emb_dim, aggr="add"):
        '''
            emb_dim (int): node embedding dimensionality
        '''
        super(GINConv_STM, self).__init__(aggr=aggr)
...
class GNN_STM(torch.nn.Module):
    def __init__(self, num_layer, emb_dim, JK="last", drop_ratio=0., gnn_type="gin"):

        if num_layer < 2:
            raise ValueError("Number of GNN layers must be greater than 1.")

        super(GNN_STM, self).__init__()
        self.drop_ratio = drop_ratio
        self.num_layer = num_layer
        self.JK = JK
        ...
        for layer in range(num_layer):
            if gnn_type == "gin":
                self.gnns.append(GINConv_STM(emb_dim, aggr="add"))
            elif gnn_type == "gcn":
                self.gnns.append(GCNConv(emb_dim))
...
class GNN_graphpred_STM(torch.nn.Module):
    """
    Extension of GIN to incorporate edge information by concatenation.
    ...
    def __init__(self, num_layer, emb_dim, num_tasks, JK, graph_pooling, molecule_node_model=None):
        super(GNN_graphpred_STM, self).__init__()
    ...
    def forward(self, *argv):
        ...
        node_representation = self.molecule_node_model(x, edge_index,
                                                       edge_attr)  # x is [4194,2],node_representaion is [4194,300]
        # print("x: {}".format(x.shape))
        # print("aaaaaaaaaaaaa",batch.shape)
        # num_nodes = scatter_add(batch.new_ones(x.size(0)), batch, dim=0,
        #                         dim_size=128)
        # print("num_nodes: {}".format(num_nodes))
        dense_x, mask = to_dense_batch(node_representation,
                                       batch)  # dense_x [batch, max_node_num, dim], mask [batch, max_node_num]

        graph_representation = self.pool(node_representation, batch)
        output = self.graph_pred_linear(graph_representation)
        return (dense_x, mask), graph_representation, output
