...
def parse_args():
    ...
    parser.add_argument('--split_mode', type=str, default='random', help='scaffold, cold, random')
    ...
    parser.add_argument('--weight_decay', type=float, default=6e-05)
    parser.add_argument('--mlp_dropout', type=float, default=0.3)
    parser.add_argument('--mlp_finger_dropout', type=float, default=0.2)
...
class AlignDataset(Dataset):
    ...
    def get_data_file(self):
        ...
        if self.split_mode == "scaffold":
            ...
            raise ValueError(f"data_file is not assigned properly. Please check the self.split_mode {self.split_mode}")
        return data_file
    ...
    def get_data(self, data_file):
        ...
        for i in data_name_list:
            ...
            self.current_ddi.append([drug1, drug2, drug1_function, drug2_function])
    ...
    def collate_fn(self, batch):
        drug1, drug2, drug1_function, drug2_function = zip(*batch)
        drug1_list = []
...
class Model_stage1(nn.Module):
    ...
    def forward_retrieval(self, batch):
        graphs1, graphs2, finger1, finger2, fun_label1, fun_label2, drug1, drug2= batch
        mol1, mask1 = self.molecule_model(graphs1)[0]  
        mol1, _, _ = self.Mol_Adapter(mol1, mask1)  
        mol1_q  = torch.matmul(mol1,self.W_q)
        mol2, mask2 = self.molecule_model(graphs2)[0]
        ...
        return  drug1, drug2, out1, out2, fun_label1, fun_label2
...
def retrieval(model, eval_dataloader, split_mode, mode, fold):
    ...
    for step, batch in enumerate(eval_dataloader):
        with torch.no_grad():
            ...
            # gt_emb_ids = gt_emb_ids+gt_id
    _ = do_compute_metrics(preds1, gt_emb_ids1)
    _ = do_compute_metrics(preds2, gt_emb_ids2)
    top_k_num = 2
    #top_k_indices1 = top_k_metric(preds1, gt_emb_ids1, top_k_num)
    #top_k_indices2 = top_k_metric(preds2, gt_emb_ids2, top_k_num)
    #assert top_k_num==0
...
def main(args):
    ...
    with open("data/drug2fingerprint.pkl", "wb") as f:
        pickle.dump(drug2fingerprint, f)
    #assert drug2fingerprint==0
