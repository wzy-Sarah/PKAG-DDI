...
def parse_args():
    parser = argparse.ArgumentParser(description='Train a cold model')
    ...
    parser.add_argument('--split_mode', type=str, default='cold', help='scaffold, cold, random')
    ...
    parser.add_argument('--learning_rate', type=float, default=0.0001)#0.0005 
    parser.add_argument('--node_cluster', type=int, default=80)
    parser.add_argument('--weight_decay', type=float, default=8e-05) #6e-05
    parser.add_argument('--mlp_dropout', type=float, default=0.2)
    parser.add_argument('--mlp_finger_dropout', type=float, default=0.3)
    parser.add_argument('--gnn_dropout', type=float, default=0.7)
    ...
    def get_data_file(self):
        if self.split_mode == "scaffold":
            ...
            if self.mode == "train":
                ...
                data_file = os.path.join(data_dir, "new_val.txt")
            elif self.mode == "test":
                data_file = os.path.join(data_dir, "new_val.txt")
            ...
            if self.mode == "train":
                ...
                data_file = os.path.join(data_dir, f"new_val_seed{self.fold}.txt")
            elif self.mode == "test":
                data_file = os.path.join(data_dir, f"new_test_seed{self.fold}.txt")
        else:
...
class Model_stage1(nn.Module):
    ...
    def forward_retrieval(self, batch):
        graphs1, graphs2, fun_label1, fun_label2,drug_name1, drug_name2 = batch
        mol1, mask1 = self.molecule_model(graphs1)[0] 
        mol1, _, _ = self.Mol_Adapter(mol1, mask1)  
        mol1_q  = torch.matmul(mol1,self.W_q)
        mol2, mask2 = self.molecule_model(graphs2)[0]
